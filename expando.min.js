var expando=function(){var h=/\#([a-z][a-z\-\_0-9]*)/i,m=/\.([a-z][a-z\-\_0-9]*)/gi,k=/^[\s]*([a-z]*)/i,l=/\*([0-9]*)/,f={node:function(){this.tag="div";this.classList=[];this.expansion=this.modifiers=this.id="";this.children=[];this.count=1},expand:function(a,d){var b="";f.treeify(a.split("")).forEach(function(a){a.expand();b+=f.EM(a,d)});return b},treeify:function(a){for(var d="",b=[],e=0;d=a.shift();)switch("undefined"===typeof b[e]&&(b[e]=new f.node),d){case "{":b[e].children=f.treeify(a);break;
...
this.id)||this.id).trim();this.tag=((a.match(k)?a.match(k)[0]:this.tag)||this.tag).trim();for(var d=this.classList=a.match(m)||[],b=0;b<d.length;b++)d[b]=d[b].trim();this.count=a.match(l)?parseInt(a.match(l)[1]):1;if(this.children)for(b=0;b<this.children.length;b++)this.children[b].expand()};return f}();
